...
def run_solution(
        dataset_train: torch.utils.data.Dataset, data_dir: str = os.curdir, output_dir: str = '/results/'
                 ) -> 'Model':
    """
...
class Model(object):
    ...
    def __init__(self):
        ...
        hidden_layers = (512, 512)  # for each entry, creates a hidden layer with the corresponding number of units
        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    # TODO 1/10: Implement Bayes by backprop training here
                    ...
                    for i in range(num_monte_carlo_samples):
                        ...
                        current_proba = F.log_softmax(current_logits, dim=1)

                        negative_log_likelihood = F.nll_loss(F.log_softmax(current_logits, dim=1),
                                                             batch_y, reduction='sum')
                        loss += negative_log_likelihood + log_variational_posterior - log_prior

                    loss = loss / num_monte_carlo_samples
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        # TODO 2/10: Create a suitable prior for weights and biases as an instance of ParameterDistribution.
        ...
        # TODO 3/10: Create a suitable variational posterior for weights as an instance of ParameterDistribution.
        ...
        if self.use_bias:
            # TODO 4/10: As for the weights, create the bias variational posterior instance here.
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        """
        # TODO 5/10: Perform a forward pass as described in this method's docstring.
        ...
        log_prior_weights = self.prior.log_likelihood(weights)
        log_variational_posterior_weights = self.weights_var_posterior.log_likelihood(weights)
        ...
        if self.use_bias:
            ...
            log_prior_bias = 0
            log_variational_posterior_bias = 0

        log_prior = log_prior_weights + log_prior_bias
        log_variational_posterior = log_variational_posterior_weights + log_variational_posterior_bias
...
class BayesNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # TODO 6/10: Perform a full pass through your BayesNet as described in this method's docstring.
        ...
        for idx, current_layer in enumerate(self.layers):
            new_features, log_prior, log_variational_posterior = current_layer(current_features)
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO 7/10: Implement this

        log_scale = math.log(self.sigma)
        log_likelihood = -((values - self.mu) ** 2) / (2 * self.sigma ** 2)\
                         - log_scale - math.log(math.sqrt(2 * math.pi))

        return torch.sum(log_likelihood)

    def sample(self) -> torch.Tensor:
        # TODO 8/10: Implement this
        eps = torch.normal(0, 1, size=self.mu.shape)
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        # Note: autograd raises issues when softplus function is used, replaced with normal computation
        # self.sigma = F.softplus(self.rho)

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO 9/10: Implement this
        log_likelihood = - np.log(np.sqrt(2 * np.pi)) - \
                         torch.log(self.sigma) - (((values - self.mu) ** 2) / (2 * self.sigma ** 2)) - 0.5

        return torch.sum(log_likelihood)

    def sample(self) -> torch.Tensor:
        # TODO 10/10: Implement this
        eps = torch.normal(0, 1, size=self.mu.shape)
        return self.mu + self.sigma * eps
