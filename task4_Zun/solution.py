...
PI_LR = 4e-3
VF_LR = 5e-3
TEMP_PARAM = 0.2    #soft actor-critic
V_UPDATE_NUM = 100
EPOCH_NUM = 50
...
#        return 0, 0, 0
        ...
        with torch.no_grad():
            ...
            value = self.v(state)
            log_prob = self.pi._log_prob_from_distribution(action_dist, action)
...
#        self.ret_buf[path_slice] = discount_cumsum(rews, self.gamma)[0] * np.ones(self.ptr-self.path_start_idx)
        self.ret_buf[path_slice] = rews[:-1] + self.gamma * vals[1:]
...
#        self.phi_buf = self.phi_buf
        mean = self.phi_buf.mean()
        std = self.phi_buf.std()
...
#        vf_lr = 1e-3
    ...
    def pi_update(self, data):
        ...
        p_loss = torch.stack(p_losses).sum() / EPOCH_NUM
        entropy = -torch.stack(log_probs).sum() / EPOCH_NUM
        loss = p_loss + TEMP_PARAM * entropy 
        p_loss.backward()
    ...
    def v_update(self, data):
        ...
        obs = data['obs']
        act = data['act']   #not used
        phi = data['phi']   #not used
        ret = data['ret']
        ...
        for _ in range(V_UPDATE_NUM):
            self.v_optimizer.zero_grad()    #this is the bug. I put it before the loop before
            predicts = self.ac.v(obs)
            v_loss = nn.functional.mse_loss(predicts, ret)
            v_loss.backward()
